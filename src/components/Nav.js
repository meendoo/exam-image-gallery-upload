...
export const DeleteButton = ({ onClick }) => {
  return (
    <Button className={styles.btnClear} onClick={onClick}>
      ...
      />{" "}
      <span className={styles.clearSpan}>Clear Gallery</span>
    </Button>
  );
};
    ...
    return (
      <nav>
        <div className={styles.actions}>
          <Button onClick={() => this.props.openUploadZoneModal()} tabIndex="0">
            Add Media
          </Button>
          <div className={styles.actionsRight}>
            ...
            <OrderButton
              order={order}
              onClick={() => this.props.handleOrder(order)}
            />
          </div>
        </div>
      </nav>
...
const mapDispatchToProps = dispatch => ({
  ...
  handleOrder: currentOrder =>
    dispatch(GalleryActions.handleOrder(currentOrder)),
